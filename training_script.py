...
def train_against_prev_iter(epochs=15, training_games_per_epoch=50, test_games_per_epoch_vs_test_players=50):
    ...
    training_players = [random1, RandomPlayer(
        hand_size, 'random-2'), defensive1]

    # define player for testing phase
    trained_player = TrainedNFSPPlayer(hand_size, 'Trained-NFSP')
    trained_player.load_from_other_player(learning_player1)
    test_players = [RandomPlayer(hand_size, "Random-2"), RandomPlayer(
        hand_size, 'Random-3'), RandomPlayer(hand_size, 'Random-4'), trained_player]
    ...
    for epoch in range(epochs):
        ...
        print('Testing vs Test Players loss ratio: ', loss_ratios)
        learning_player1.save_network(
            'train_against_prev_iter/epoc-' + str(epoch + 1))

        prev_iter1 = TrainedNFSPPlayer(hand_size, 'prev-iter1')
        prev_iter1.load_model(
            'train_against_prev_iter/epoc-'+str(random.randint(1, epoch + 1)))
        training_players = [random1, prev_iter1]

        if random.random() >= 0.5:
            prev_iter = TrainedNFSPPlayer(hand_size, 'prev-iter')
            prev_iter.load_model('train_against_prev_iter/epoc-' +
                                 str(random.randint(max(1, epoch - 10), epoch + 1)))
        ...
        if epoch % 10 == 0:
            ...
            learning_player2.load_network_from_other_by_reference(
                learning_player1)
...
def train_against_prev_iter_test_aginst_def(epochs=50, training_games_per_epoch=25, test_games_per_epoch_vs_test_players=50):
    ...
    trained_player = TrainedNFSPPlayer(hand_size, 'Trained-NFSP')
    trained_player.load_from_other_player(learning_player1)
    test_players = [DefensivePlayer(hand_size, 'Defensive-1'), DefensivePlayer(
        hand_size, 'Defensive-2'), DefensivePlayer(hand_size, 'Defensive-3'), trained_player]
...
def train_against_one_random(epochs=50, training_games_per_epoch=50, test_games_per_epoch_vs_test_players=50):
    ...
    for epoch in range(epochs):
        ...
        print('Testing vs Test Players loss ratio: ', loss_ratios)
        learning_player1.save_network(
            'train_against_prev_iter/epoc-' + str(epoch))
        ...
        try:
            prev_iter2.load_model(
                'train_against_prev_iter/epoc-' + str(np.argmax(loss_vector)))
        except:
            prev_iter2.load_model(
                'train_against_prev_iter/epoc-' + str(np.argmax(loss_vector))[-1])
        prev_iter3 = TrainedNFSPPlayer(hand_size, 'prev-iter')
        prev_iter3.load_model(
            'train_against_prev_iter/epoc-' + str(random.randint(0, epoch)))
        training_players = [DefensivePlayer(
            hand_size, 'a'), DefensivePlayer(hand_size, 'b'), prev_iter3]
        ...
        # save the network
        learning_player1.save_network(
            'train_against_two_randoms/epoc-' + str(epoch))
